...
```
After reading JSON data, I used flatten() to eliminate the structure, converting them to data frame and saved as "yelp_review.csv" and "yelp_business.csv".
...
This is the bar plot of table above. I sorted the ratings in the table in decreasing order. According to the plot, El Dorado Hills(EDH) in the US, Baden-Württemberg(BW) in German and Quebec(QC) in Canada have highest scores. Although Ontaria(ON) has the highest number of restaurants, it has relatively lower ratings. 
...
Here's some interesting findings for this part:
 ...
 - El Dorado Hills(EDH) in the US, Baden-Württemberg(BW) in German and Quebec(QC) in Canada have highest rating scores. Probably they are the right places to find most delicious food.
